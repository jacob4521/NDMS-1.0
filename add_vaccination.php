...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $administeredBy = $_POST['administered_by'];
    $notes = $_POST['notes'];
    ...
    if ($citizenResult->num_rows == 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO VaccinationRecords (CitizenID, VaccineName, DoseNumber, DateAdministered, AdministeredBy, Notes) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("isisss", $citizenId, $vaccineName, $doseNumber, $dateAdministered, $administeredBy, $notes);
        
        if ($stmt->execute()) {
            $msg = "Vaccination record added successfully for Citizen eID: $citizenEID";
        } else {
...
$recentRecords = $conn->query("
    SELECT vr.*, c.FirstName, c.LastName, c.Citizen_eID
...
$vaccines = $conn->query("SELECT VaccineName, Description FROM Vaccines ORDER BY VaccineName ASC");
?>
<!DOCTYPE html>
<html>
<head>
    <link rel="stylesheet" href="includes/sidebar.css">
    <title>Vaccination Records Management - NDMS</title>
    <style>
...
<body>
    ...
    <div class="container">
        ...
        <div class="header">
            ...
            <p>Comprehensive immunization tracking and management system</p>
        </div>
        
        <!-- Navigation -->
        <div class="nav-bar">
            <a href="dashboard.php">🏠 Dashboard</a>
            <a href="add_medical.php">🏥 Add Medical Record</a>
            ...
            <form method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="vaccine_name" id="vaccine_name" required>
                            ...
                            <?php while($vaccine = $vaccines->fetch_assoc()): ?>
                                <option value="<?= htmlspecialchars($vaccine['VaccineName']) ?>">
                                    <?= htmlspecialchars($vaccine['VaccineName']) ?>
                                    <?php if ($vaccine['Description']): ?>
                                        - <?= htmlspecialchars($vaccine['Description']) ?>
                                    <?php endif; ?>
                                </option>
                            <?php endwhile; ?>
                        </select>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select name="dose_number" id="dose_number" required>
                            ...
                            <option value="6">Booster</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="date_administered">Date Administered:</label>
                        <input type="date" name="date_administered" id="date_administered" required>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="administered_by" id="administered_by" required placeholder="Doctor/Nurse name">
                    </div>
                </div>

                <div class="form-group">
                    <label for="notes">Notes (Optional):</label>
                    <textarea name="notes" id="notes" placeholder="Any additional notes, side effects, or observations..."></textarea>
                </div>
                ...
                <tbody>
                    <?php while($record = $recentRecords->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars($record['VaccineName'] ?? '') ?></td>
                            <td><?= htmlspecialchars($record['DoseNumber'] ?? '') ?></td>
                            <td><?= $record['DateAdministered'] ?? '' ?></td>
                            <td><?= htmlspecialchars($record['AdministeredBy'] ?? '') ?></td>
